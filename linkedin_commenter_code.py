    ...
    try:
        ...
        driver = webdriver.Firefox( 
            service=Service(
                    ...
                    options=options)
        driver.set_page_load_timeout(600000000)
        ...
        while True:
            ...
            send_telegram_message(group_id=telegram_group_id,
                                message_text="Starting a new run!",
                                api_url=API_URL)
            ...
            for i in range(20):
                try:
                    ...
                    send_telegram_message(group_id=telegram_group_id,
                                        message_text=f"Cant find notification element - {xpath}",
                                        api_url=API_URL)
            ...
            send_telegram_message(group_id=telegram_group_id, message_text="\n\n".join(posts_to_comment_urls), api_url=API_URL)
            if len(posts_to_comment_urls) > 0:
                for url in posts_to_comment_urls:
                    try:
                        ...
                        while True:
                            try:
                                try:
                                    ...
                                    comment_element = driver.find_element(By.XPATH, '//button[@class="comments-comment-box__submit-button--cr artdeco-button artdeco-button--1 artdeco-button--primary ember-view"]')
                                    driver.execute_script("arguments[0].click();", comment_element)
                                time.sleep(2)
                                break
                            except:
                                driver.find_element(By.CLASS_NAME, 'msg-overlay-bubble-header__control artdeco-button artdeco-button--circle artdeco-button--muted artdeco-button--1 artdeco-button--tertiary ember-view').click()
                        ...
                        message = f"Error trying to comment on post: \n\n {post_text}\n\n with comment:\n\n{comment_text}\n\n This is the error: {e}"
                        print(message)
                        send_telegram_message(
                            group_id=telegram_group_id, 
                            message_text=message, 
        ...
        send_telegram_message(
                            group_id=telegram_group_id, 
                            message_text=f"Rerunning the code. Please check this error - {e}!!", 
                            api_url=API_URL)
